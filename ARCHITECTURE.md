...
1. **Il designer non ridisegna i controlli.** Usa una WebView embedded dentro Avalonia che carica il vero progetto Blazor dell'utente, servito localmente da `BlazorAppHost` (build esplicita + server di file statici persistente — vedi sezione 5, non più `dotnet watch`), in modalità `?design=true`. Ciò che si vede nel designer È l'app reale.
...
- **Un componente e' una coppia indivisibile `.cs`+`.razor` (2026-08-27)**: la classe Visual (dati/proprieta') da sola non basta, serve il componente Blazor `.razor` corrispondente a renderizzarla davvero. Trovato un bug reale dopo aver introdotto l'editor sorgente dal doppio click sulla Toolbox (modulo 16): "Salva come nuovo componente" duplicava solo la classe Visual, producendo un tag Toolbox senza nessun componente Blazor a renderizzarlo. Deciso esplicitamente con l'utente (fra fondere i due file in uno solo - il `.razor` supporta classi extra nel blocco `@code{}` - e mantenerli separati ma "atomici") di **mantenerli due file separati ma trattati sempre come un'unita' da ogni strumento dell'IDE**: `ComponentPluginLoader` esclude e segnala come errore una classe Visual senza il suo `.razor` (invece di lasciarla entrare in Toolbox rotta in silenzio); l'editor sorgente (`ComponentEditorWindow`) mostra/salva/duplica sempre entrambi insieme, mai uno senza l'altro. Fondere i due file in uno solo resta un'opzione valutata per il futuro se questa fragilita' dovesse ripresentarsi, ma richiederebbe riscrivere `ComponentPluginLoader` per compilare il blocco `@code` di un `.razor` invece di un `.cs` puro - cambiamento piu' invasivo, rimandato.
- **Evidenziazione sintattica nell'editor sorgente**: `Avalonia.AvaloniaEdit` 12.0.0 (attenzione al nome del pacchetto: il semplice `AvaloniaEdit` su NuGet e' una serie ferma alla 0.10.x, non allineata ad Avalonia 12 - quello giusto e' `Avalonia.AvaloniaEdit`). Per il `.cs` usa la definizione "C#" gia' integrata; per il `.razor` (markup HTML + C# nel blocco `@code{}`, nessuna definizione integrata per Razor) una definizione XSHD custom (`RazorHighlighting`) che riusa la stessa tecnica della definizione integrata "ASP/XHTML" (pensata per ASP.NET classico, HTML + `<% %>` di C#): importa i ruleset gia' registrati "HTML/" e "C#/" invece di reimplementarli, con `@code\s*\{` / `^\}` al posto di `<%` / `%>`. Euristica, non un vero parser Razor (l'end-span assume che il blocco `@code` si chiuda con una graffa da sola a inizio riga, come fanno gia' tutti i componenti esistenti) - verificata funzionante in un harness isolato prima di integrarla.
...
- **Incompatibile con `PublishSingleFile`** (verificato empiricamente, 2026-08-26): in single-file, self-contained bundla anche il runtime .NET nell'eseguibile, e `Assembly.Location` ritorna stringa vuota per QUALUNQUE assembly incorporato (non solo `Ide.Designer`/`VbControls` - anche `System.Private.CoreLib` e tutto il resto). `Directory.GetFiles(RuntimeEnvironment.GetRuntimeDirectory(), ...)` e `AppDomain.CurrentDomain.GetAssemblies().Select(a => a.Location)` smettono di produrre riferimenti validi per Roslyn: la compilazione di `Components/*.cs` fallisce perfino su `System` (256 errori, 0 componenti, testato). La pubblicazione di release resta quindi una cartella self-contained "sciolta" (molti file, incluse le DLL runtime) finche' `ComponentPluginLoader` dipende da percorsi fisici di assembly per le `MetadataReference` — vedi `scripts/package-release.sh`.
- **Controlli comuni condivisi**: `VbButton`, `VbLabel`, `VbTextBox`, `VbTextArea`, `VbHttpClient`, `VbTimer`, `VbLocalStorage` (coppie `.razor` + `*Visual.cs`) vivono in `src/VbControls/Components/`, non duplicati per progetto. Ogni progetto Blazor (`BlazorPwaTemplate`, `RssFeedViewer`, futuri) li include via `<RazorComponent Include>`/`<Compile Include>` con lo stesso pattern glob gia' usato per `VbControls`/`VbControls.Abstractions` (ogni `.razor` dichiara esplicitamente `@namespace VbControls`, quindi la posizione fisica non altera il namespace risolto). `ComponentPluginLoader.Load(params string[] componentsDirectories)` compila insieme la cartella `Components/` del progetto aperto e questa cartella comune, cosi' i controlli comuni sono sempre disponibili in Toolbox indipendentemente dal progetto (`MainWindow.LoadComponents`).
- **Verifica di conformita' all'interfaccia**: oltre a `IsAssignableFrom`/`!IsAbstract`, `ComponentPluginLoader.VerificaFirmaComponente` controlla esplicitamente via reflection che ogni membro di `IDesignComponent` sia implementato con la firma corretta (nome, parametri, tipo di ritorno). Nel percorso da sorgente qui descritto e' gia' garantito dal compilatore C# (una classe concreta non puo' implementare un'interfaccia solo in parte) e quindi non esclude mai nulla in pratica — resta come rete di sicurezza a basso costo, pensata per il giorno in cui si caricheranno anche assembly gia' compilati (DLL), dove non c'e' un compilatore a garantire la conformita'. Non e' un meccanismo di sandboxing: non ispeziona ne' limita cosa il codice del plugin fa a runtime, resta valida la nota di fiducia sopra.
...
- WebView: **`Avalonia.Controls.WebView` (pacchetto ufficiale AvaloniaUI OÜ), versione `12.1.0`** — validato in Fase 0 (2026-08-25).
  ...
  - **Nota operativa (gotcha riscontrato, poi superato)**: `dotnet watch` avviava anche un proprio *browser-refresh server* che prova a esporre un endpoint HTTPS interno; se manca il certificato di sviluppo fallisce con `Unable to configure HTTPS endpoint`. Soluzione adottata inizialmente: avviare con `DOTNET_WATCH_SUPPRESS_BROWSER_REFRESH=1 dotnet watch --urls http://localhost:5245` (più semplice di `dotnet dev-certs https --trust`, dato che qui la pagina è consumata dalla WebView e non da un browser con auto-refresh).
  - **`dotnet watch` abbandonato (2026-08-27)**: quella stessa soppressione del browser-refresh, su una build reale su Windows, faceva terminare `dotnet watch` con una `NullReferenceException` in `BlazorWebAssemblyDeltaApplier.WaitForProcessRunningAsync` — l'applier di Hot Reload per Blazor WASM aspetta una connessione dal browser per applicare le modifiche "in place", connessione che col refresh disattivato non arriva mai. Il fix (`--no-hot-reload`) introduceva a sua volta un secondo bug: senza Hot Reload, OGNI modifica forza un riavvio completo del processo figlio, riavvio che va spesso in race sui file compilati (osservato empiricamente, riproducibile su modifiche successive alla prima). Dato che il modello di sincronizzazione è ormai manuale (bottone "Aggiorna" — vedi modulo 15 sotto), non serve più un file-watcher generico: `dotnet watch` è stato sostituito da `BlazorAppHost` (`src/Ide.Designer`), che fa una build esplicita one-shot (`dotnet build`, vero exit code) e serve l'output con un server di file statici persistente (`blazor-devserver.dll`, dal pacchetto NuGet `Microsoft.AspNetCore.Components.WebAssembly.DevServer` — già una dipendenza del template, lo stesso eseguibile che `dotnet watch` lanciava come proprio processo figlio) avviato una sola volta e mai riavviato, quindi mai in race con se stesso.
  - **Limite noto del sandbox Linux di sviluppo (non del codice)**: in questo ambiente, `Process.WaitForExitAsync`/`HasExited` di .NET non si accorgono MAI dell'uscita di un `dotnet build` lanciato da dentro `Ide.App` (verificato: lo stesso comando lanciato da una console app standalone viene rilevato in ~2.7s), pur completando il lavoro reale in pochi secondi (file di output scritti regolarmente). Ipotesi più probabile: GTK (backend Linux di `Avalonia.Controls.WebView`) intercetta la notifica di terminazione del processo figlio (SIGCHLD/waitpid) prima che .NET possa osservarla — non dovrebbe presentarsi su Windows (WebView2, attesa di uscita processo via handle nativo, non SIGCHLD). Mitigato con un timeout+kill da 60s in `BlazorAppHost` (stessa rete di sicurezza già usata da `ProjectPublisher` per `dotnet publish`, che soffre dello stesso sintomo).
...
5. Integrazione WebView + avvio del progetto Blazor in background (`BlazorAppHost`, originariamente `dotnet watch` — abbandonato, vedi sezione 4)
...
16. Editor sorgente componenti dal doppio click sulla Toolbox (sezione 2.2): coppia `.razor`+`.cs` trattata come unita' indivisibile (`ComponentEditorWindow`, `ComponentSourceEditor`), evidenziazione sintattica C#/Razor (`RazorHighlighting`)
17. Nuovo/Apri/Salva Progetto + Proprieta' progetto + Undo/Redo: `ProjectStateStore` (`src/Ide.Designer/ProjectState.cs`) persiste lo stato di design (`project.ide.json`, auto-salvato ad ogni `RegenerateFiles`) separato dai file Blazor derivati; `UndoRedoManager` (stesso file di stato, a snapshot) copre piazzamento/cancellazione/riordino/modifica proprieta' dei controlli - non il testo dei metodi in `{Form}.Behavior.cs`, fuori scope esplicito. "Nuovo Progetto" copia lo scheletro del template incluso in una cartella scelta dall'utente (stesso `.csproj`/namespace del template, non rinominati - fuori scope per ora una vera generalizzazione multi-progetto)
